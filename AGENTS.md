...
## Codex Behaviour for This Project
...
> Codex must never send emails, post Slack messages, or merge PRs without explicit user approval in the session.
...
### Progress Tracker
- [ ] Phase 1 — `feature/llm-provider` ← start here
...
Mark phases `[x]` as they are merged to master. When starting a session, Codex reads the first unchecked phase as the current one.
...
Active provider is set in `appsettings.json` under `Ai.Provider`. API keys go in `appsettings.local.json` (gitignored). `LlmProviderFactory.Create(settings)` is called in the `MainForm` constructor alongside existing service wiring.

**Model tiers** — services request a capability tier, not a hardcoded model string:
- `Fast` — Haiku / gpt-4o-mini / gemini-2.0-flash (suggestions, debounced, low-latency tasks)
- `Balanced` — Sonnet / gpt-4o / gemini-2.5-pro (profile building, query review)
- `Smart` — reserved; not currently used

Model strings per tier are configured in `appsettings.json` under `Ai.Models`. When the active provider changes, only the model strings in config change — service code stays the same.
...
"Ai": {
  "Provider": "Anthropic",
  "Models": {
    "Fast":     "Codex-haiku-4-5-20251001",
    "Balanced": "Codex-sonnet-4-20250514",
    "Smart":    "Codex-sonnet-4-20250514"
  },
  "ApiKeys": {
    "Anthropic": "",
    "OpenAI":    "",
...
Build `ILlmProvider`, all three provider implementations, `LlmProviderFactory`, and the request/response DTOs. Wire the factory into `MainForm` constructor. Add `InMemoryLlmProvider` test double for use in all AI service tests. Add a status label to the form footer showing active provider and model (e.g. "Anthropic · Codex-sonnet-4-20250514").
...
- Hardcode a model ID string (e.g. `"Codex-sonnet-4-20250514"`) in a service class — use `ModelTier`
